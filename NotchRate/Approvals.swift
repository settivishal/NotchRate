...
final class Approvals {
    ...
    /// Terminal came to the front: the finished answers have been seen.
    func clearDone() {
        guard done else { return }
        for f in (try? FileManager.default.contentsOfDirectory(at: directory, includingPropertiesForKeys: nil)) ?? [] where f.lastPathComponent.hasPrefix("done-") {
